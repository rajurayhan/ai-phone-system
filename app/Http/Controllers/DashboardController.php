...
class DashboardController extends Controller
{
    public function stats(Request $request)
    {
        ...
        // Get basic counts
        $totalAssistants = Assistant::where('user_id', $user->id)->count();
        $totalCalls = CallLog::where('user_id', $user->id)->count();
        ...
        $callsThisMonth = CallLog::where('user_id', $user->id)
            ->where('start_time', '>=', $thisMonth)
            ->count();
        
        // Calculate total minutes
        $totalMinutes = CallLog::where('user_id', $user->id)
            ->whereNotNull('duration')
            ->sum('duration') / 60; // Convert seconds to minutes
        
        $thisMonthMinutes = CallLog::where('user_id', $user->id)
            ->where('start_time', '>=', $thisMonth)
            ->whereNotNull('duration')
            ->sum('duration') / 60; // Convert seconds to minutes
        
        // Calculate average call duration
        $avgDuration = CallLog::where('user_id', $user->id)
            ->whereNotNull('duration')
            ->avg('duration');
        ...
        $callTrends = CallLog::where('user_id', $user->id)
        ...
        $assistantStats = Assistant::where('user_id', $user->id)
    ...
    public function activity(Request $request)
    {
        ...
        $recentAssistants = Assistant::where('user_id', $user->id)
            ->orderBy('created_at', 'desc')
            ->limit(5)
            ->get()
            ->map(function($assistant) {
    ...
    public function adminStats(Request $request)
    {
        ...
        // Get today's data
        $today = Carbon::today();
        $callsToday = CallLog::whereDate('start_time', $today)->count();
        $newUsersToday = User::whereDate('created_at', $today)->count();
        $newAssistantsToday = Assistant::whereDate('created_at', $today)->count();
        
        // Get this month's data
        $thisMonth = Carbon::now()->startOfMonth();
        $callsThisMonth = CallLog::where('start_time', '>=', $thisMonth)->count();
        ...
        // Calculate total minutes
        $totalMinutes = CallLog::whereNotNull('duration')->sum('duration') / 60; // Convert seconds to minutes
        $thisMonthMinutes = CallLog::where('start_time', '>=', $thisMonth)
            ->whereNotNull('duration')
            ->sum('duration') / 60; // Convert seconds to minutes
        
        // Calculate average call duration
        $avgDuration = CallLog::whereNotNull('duration')->avg('duration');
        ...
        $recentCalls = CallLog::with(['user', 'assistant'])
            ->orderBy('start_time', 'desc')
            ->limit(10)
            ->get();
        ...
        // Get call trends (last 30 days)
        $callTrends = CallLog::where('start_time', '>=', Carbon::now()->subDays(30))
            ->selectRaw('DATE(start_time) as date, COUNT(*) as count')
            ->groupBy('date')
            ->orderBy('date')
            ->get();
        ...
        $userGrowthTrends = User::where('created_at', '>=', Carbon::now()->subDays(30))
    ...
    public function adminActivity(Request $request)
    {
        ...
        // Recent assistant creations
        $recentAssistants = Assistant::with('user')
            ->orderBy('created_at', 'desc')
            ->limit(10)
            ->get()
            ->map(function($assistant) {
