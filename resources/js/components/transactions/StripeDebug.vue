...
export default {
  ...
  computed: {
    stripeKeyAvailable() {
      return !!process.env.MIX_STRIPE_PUBLISHABLE_KEY
    },
    stripeKeyLength() {
  ...
  methods: {
    ...
    async testStripe() {
      try {
        ...
        const envCheck = {
          keyAvailable: !!process.env.MIX_STRIPE_PUBLISHABLE_KEY,
          keyLength: process.env.MIX_STRIPE_PUBLISHABLE_KEY?.length || 0,
          keyPrefix: process.env.MIX_STRIPE_PUBLISHABLE_KEY?.substring(0, 20) || 'N/A'
        }
